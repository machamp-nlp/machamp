...
def get_range_vector(size: int, device: int) -> torch.Tensor:
    ...
    """
    if device > -1:
        return torch.ones(size, device=device, dtype=torch.long).cumsum(0) - 1
    else:
...
class MachampDepDecoder(MachampDecoder, torch.nn.Module):
        ...
        if type(gold_head_tags) != type(None):
            ...
            if self.additional_metrics:
                for additional_metric in self.additional_metrics:
                    additional_metric.score(predicted_heads, predicted_head_tags, gold_head_indices, gold_head_tags, mask)
    ...
    def _greedy_decode(
        ...
        if self.get_top_n:
            ...
            topn = min(self.topn, head_tag_logits.shape[-1])
            topk_labels = torch.topk(F.softmax(head_tag_logits, -1), topn, dim=2)
