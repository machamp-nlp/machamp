...
class WordpieceIndexer(TokenIndexer[int]):
    ...
    def tokens_to_indices(self,
        ...
        # Flag indicating the task is token level or not
        # Hack: take property isWordLevel from entity tag!
        is_word_level = [token.ent_type_ for token in tokens][0]
        task_level_ids = [0 if is_word_level else 1]
        ...
        # TODO: Assume separator_ids consists of only 1(0th) item
        if not is_word_level:
            tokens_a = flat_wordpiece_ids
            if self._separator_ids[0] in tokens_a:
                sep_idx = flat_wordpiece_ids.index(self._separator_ids[0])
                tokens_a = flat_wordpiece_ids[:sep_idx]
                tokens_b = flat_wordpiece_ids[sep_idx+1:]
            else:
                tokens_b = []
            max_pieces = self.max_pieces - 3 if len(tokens_b) > 0 else self.max_pieces - 2
            if len(flat_wordpiece_ids) > max_pieces:
                self._truncate_seq_pair(tokens_a, tokens_b, max_pieces)
            flat_wordpiece_ids = tokens_a + self._separator_ids + tokens_b if len(tokens_b) > 0 else tokens_a
        ...
        token_type_ids = _get_token_type_ids(wordpiece_ids, self._separator_ids, self.max_pieces)
        ...
        return {
            ...
            "mask": mask,
            f"{index_name}-task-level-ids": task_level_ids,
        }
...
@TokenIndexer.register("machamp-bert-pretrained")
...
def _get_token_type_ids(wordpiece_ids: List[int],
                        separator_ids: List[int], max_pieces: int) -> List[int]:
...
class BertEmbedder(TokenEmbedder):
    ...
    def forward(self,
                ...
                task_level_ids: torch.LongTensor = None) -> torch.Tensor:
        """
        Parameters
        ----------
        input_ids : ``torch.LongTensor``
        ...
        if needs_split:
            ...
            # Do exactly the same for the split_token_type_ids
        ...
        is_word_level = (task_level_ids[0].item() == 0)
...
@TokenEmbedder.register("machamp-bert-pretrained")
class MachampPretrainedBertEmbedder(BertEmbedder):
...
@TokenEmbedder.register("machamp-bert-predictor")
